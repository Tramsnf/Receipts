...
├── examples/                             ← runnable demos (verified end-to-end)
│   ├── quickstart-node/                  ← Express + pino + pino-roll, writes logs/app.YYYY-MM-DD.N.log
│   ├── quickstart-python/                ← FastAPI + structlog + TimedRotatingFileHandler
│   └── quickstart-go/                    ← net/http + log/slog + lumberjack (Go 1.21+)
└── assets/
...
- **Default for local dev** (per the cookbooks): structured JSON to **stdout AND** a rotated file under `./logs/`. See the three runnable demos in [`examples/`](examples/) — Node ([`quickstart-node`](examples/quickstart-node/)), Python ([`quickstart-python`](examples/quickstart-python/)), Go ([`quickstart-go`](examples/quickstart-go/)). Each runs in 30 seconds and writes to a real file you can `tail -f`.
